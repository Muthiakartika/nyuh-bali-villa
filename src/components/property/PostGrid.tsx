...
import { formatPostDate } from "@/components/property/PostBody";
  ...
  return (
    <article className="h-full">
      <Link href={post.path} className="group/card flex h-full flex-col">
        ...
        <p className="text-eyebrow font-body mt-5 text-primary-deep uppercase">
          <time dateTime={post.date}>{formatPostDate(post.date)}</time>
        </p>

        <h3 className="font-heading mt-3 text-[19px] leading-snug font-light text-ink transition-colors duration-300 group-hover/card:text-primary-deep md:text-[21px]">
          {post.title}
        </h3>

        <span
          aria-hidden
          className="mt-4 block h-px w-9 bg-primary transition-all duration-500 ease-out group-hover/card:w-20"
        />
      </Link>
    </article>
  );
      ...
      {lead ? (
        <Reveal>
          <article className="mt-8 md:mt-10">
            ...
            >
              <div className="relative h-64 w-full overflow-hidden sm:h-80 lg:col-span-7 lg:h-[25rem]">
                <Image
                  src={lead.image}
                  alt={lead.title}
                  fill
              ...
              <div className="lg:col-span-5">
                ...
                <h3 className="font-heading mt-4 text-[26px] leading-tight font-light text-ink transition-colors duration-300 group-hover/lead:text-primary-deep md:text-[32px]">
                  {lead.title}
                </h3>
                ...
                {lead.excerpt ? (
                  <p className="mt-5 text-[16px] leading-[1.7] font-light text-text">
                    {lead.excerpt}
                  </p>
                ) : null}

                <span className="text-eyebrow font-body mt-6 inline-block text-primary-deep uppercase underline decoration-primary/40 underline-offset-[6px] transition-colors duration-300 group-hover/lead:decoration-primary">
                  Read more
                </span>
              </div>
            </Link>
