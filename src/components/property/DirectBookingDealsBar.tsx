...
type DirectBookingDealsBarProps = {
  bookingHref: string;
  /** The offer line — "Direct Booking Deals 66% Off". */
  headline: string;
  /** The promo code, set in gold under the headline. Empty omits the line. */
  code?: string;
  buttonLabel: string;
  dismissLabel: string;
};
 ...
 *
 * Presentation only — every word it shows is a prop. The wording used to be
 * three literals here, including the discount and the promo code, on a bar
 * that renders on all 78 pages: a client running a different offer needed a
 * developer, and the same code already existed as an editable field on the
 * About band, so the two could disagree. `DirectBookingDeals` resolves them
 * from Site settings and renders this.
 *
 ...
 */
export function DirectBookingDealsBar({
  bookingHref,
  headline,
  code,
  buttonLabel,
  dismissLabel,
}: DirectBookingDealsBarProps) {
