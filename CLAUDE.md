...
- **Bands:** compose with `ui/Section` (`tone` × `space`), never hand-rolled padding. Totals per step are 52/80px (`tight`), 60/96px (`normal`), 68/112px (`loose`), mobile/`md`. These were **tuned down repeatedly** from `py-20 md:py-28`, each time because the client read the gaps as too wide. Heading→content inside a band is `mt-8 md:mt-10`.
...
- **A band's top and bottom padding are deliberately unequal — 8px more on the bottom at `md`, 12px on mobile** (`normal` is `pt-6 pb-9 md:pt-11 md:pb-13`, not `py-*`). Bands open on a heading and close on a hard edge (photographs, a plate, a button). Two effects stack: a heading's glyphs start ~7px below their line box, and a strip bounded by a *dark photograph* reads tighter than the same strip bounded by text on the same background. The client called the phone's bottom strip too narrow while it measured wider than the top one — so on mobile the correction is optical (12px), not arithmetic. Don't "fix" it back to symmetry. `AboutNarrative` is the one exception to the whole rule (its `pt` is clearance under the overlapping booking card, not rhythm).
