...
- **Dark is punctuation, not the baseline.** The old design ran **seven** full-width `ink` slabs per page, three of them consecutive at the foot. It's now **one** dark `<main>` section (TestimonialCarousel) plus the awards row + footer as one dark base — the "Best Price Guaranteed" offer, once its own dark band, is now a dark *plate* folded into the light About section (see `AboutNarrative`). Verify with: count `main > section` whose computed background is `rgb(38, 30, 19)` — it should be 1.
...
- **Header:** transparent over a hero **with a brown scrim gradient + gold hairline** (`ink/90 → transparent`, opacity cross-dissolves on scroll — *not* frosted glass), solid `ink` + compacted height on scroll (`overlay` prop). Logo enlarged (hero 154×56 / scrolled 128×46) to read on the dark bar. Pages with no hero pass no `overlay` and get a solid `sticky` bar. The header carries the **Book Now CTA**.
- **`PropertyHero`** is navigated by **gold bullet indicators** (one centred row), not prev/next arrows.
- **`BookingSearchBar` overlaps the hero's bottom edge**, framed in a gold-brown hairline (`border-primary/45`, the live-widget look). It carries the site's **one** shadow; everything else stays flat.
