...
- Home page (`src/app/page.tsx`) two-panel picker is the only remaining place still using `px-5 md:px-[92px]` rather than `Container` — left alone deliberately because it's a distinct 2-panel side-by-side layout, not a standard content section. Worth measuring against the live homepage before changing.
