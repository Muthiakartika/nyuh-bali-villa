...
**`next.config.ts` carries 95 permanent redirects in four sets, and the first two exist for the same reason: a URL WordPress publishes today that nothing here would answer.** A 308 keeps the ranking a page has accumulated; a 404 throws it away.
...
**`ALLOW_ALIAS_REVIEW=1` is the escape hatch for the window this project is actually in.** The build is finished while `nyuhbalivillas.com` still resolves to WordPress, so the production alias is the only address at which the finished site can be looked at — and the default 308 sends a reviewer straight to the site they are trying to replace. A preview deployment is not a substitute, because the thing under review is the production build. Set, the alias serves the page and answers **`X-Robots-Tag: noindex, nofollow`** instead of redirecting; `noindex` is a directive rather than the hint a canonical is, so the alias is no more indexable than it is behind the 308, and what it gives up — links to a `*.vercel.app` address passing their signals on — is nothing anybody has. **So forgetting to unset it at cutover is not an incident**, which is the whole reason it is written as a swap rather than an off switch. Verified both ways against a production build: unset, the alias 308s to the canonical origin with the query string intact while `cf-ray` and `/api/` stay 200; set, the same request is 200 with the header, and neither the canonical host nor a `cf-ray` request carries it. And **never pair it with `Disallow: /`** — a blocked page is never fetched, so its `noindex` is never read, and an already-indexed URL then has no way to be removed.
